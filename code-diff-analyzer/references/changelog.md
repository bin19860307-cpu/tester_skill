...
- **2026-09-20 · v1.1.3 · 文档可用性重构 + 报告效果实证 + P2 健壮性（回应 SkillHub 评测 A/C 低分）**
  - 背景：SkillHub 五维评测 **T 5.0 / R 4.5 / A 4.4 / C 4.4 / E 4.7**，短板集中在文档与稳定性——A·触发方式 4.3、C·反模式与 FAQ 4.3、C·渐进式披露 4.3、C·结构清晰 4.4、R·运行稳定性 4.3。核心问题是主文档 **1257 行过长、细则与规则混排、缺"报告长什么样"与集中 FAQ**。本次为一次**纯文档 + 轻健壮性**改动，不改任何分析口径。
  - ① **SKILL.md 瘦身重构（1257 → 453 行）**：新增七个面向使用者的章节——「能力边界与适用性」（✅适用 / ❌不适用 / 📏规模与限制三栏）、「触发条件」（必触发 / 按需触发 / 不触发三层表）、「分析流程总览」（Step 1–7 必做 / 按需标注 + 最小路径 / 完整路径 + ASCII 流程）、「脚本编排与数据流」（编排 4 / 共 12 + ASCII 数据流 + 基础能力模块表）、「输出与报告示例」（内嵌 2 张真实报告截图）、「高频用法」（recipe 速查表）、「常见问题（速查）」。原 Step 5b/6/7 细则、流程 A/B/C、CI 章节**逐字下沉**至 references，主文档只留规则 + 流程 + 指针。
  - ② **新增 6 个 references 分册**（内容逐字迁移、不改写）：`analysis-rules.md`（Step 1/3/5b 规则）、`data-sink.md`（Step 6 数据沉淀细则）、`report-format.md`（Step 7.1/7.2）、`report-injection.md`（HTML 生成与增强层注入）、`flows-abc.md`（流程 A/B/C）、`ci-pipeline.md`（CI / 容器化）。渐进式披露：主文档瘦身、细则按需加载。
  - ③ **新增 `references/faq.md`（约 20 条 Q&A）**：按「现象 → 原因 → 处理」组织，覆盖报告裸文本泄漏、报告分与趋势分不一致、Bug 关联数为 0、解压后链接打不开、运行期裸露 traceback、脚本超时等高频问题，把散落各处的排障经验集中成一页。
  - ④ **新增 2 张真实产物报告截图**（`report-sample-1.png` 单服务报告 / `report-sample-2.png` 综合报告），回应「缺"报告长什么样"的直观示例」。**素材来自真实报告，已脱敏**：作者名、内网仓库路径、平台名等均已替换（投稿包会统一按脱敏规则处理），脱敏后残留扫描 0 命中；因为 PNG 不经过文本脱敏管线，故在进入源目录前先就地脱敏再渲染，避免敏感信息随二进制文件进入投稿包。
  - ⑤ **P2 健壮性（R·运行稳定性）**：`pipeline_wrapper.py` 增加子脚本执行超时（默认 600s）与 `git clone` 超时（默认 180s）保护，新增 `--timeout` / `--git-timeout` 开关；超时给出可读提示（"磁盘 / 网络卡顿或输入过大，可调大后重试"）而非挂死。`cdx_errors.read_text` / `read_json` 的 OSError 提示补充「网络盘 / 云盘读取超时」成因。
  - 验证：全 **16** 个脚本 `py_compile` 通过；`pipeline_wrapper.py --help` 正常显示新参数；SKILL.md 内所有 references 链接有效；截图脱敏残留 0 命中。**无分析口径变更**，v1.1.2 及以前的行为完全兼容。
